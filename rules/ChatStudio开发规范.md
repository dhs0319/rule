...
### 20.6 UI规范
- 整体设计简约美观，避免在同一个页面内出现过多的颜色，风格尽量统一
- 禁止使用蓝紫配色的夜间模式，禁止claude风格（米白底 + 暖橙 accent、复古 serif 标题）
- 禁止大角度rounded cards且禁止无目的嵌套边框、卡片，同时克制阴影
---
...
1. 初始化 React、TypeScript、Vite 和 Manifest V3 项目
2. 配置 Side Panel 和 Service Worker 多入口构建
3. 建立类型定义和 Runtime 通信协议
4. 建立 Dexie 数据库和迁移机制
5. 完成会话和消息基础 CRUD
6. 完成一个 OpenAI 兼容 Provider
7. 完成普通请求
8. 完成流式请求、停止和失败恢复
9. 完成 Markdown 安全渲染
10. 完成模型配置和连接测试
...
- 可构建的 Chrome 扩展项目，支持Chromium内核的浏览器
